...
void shiftBy(char **argv, int s, int by){
    while(argv[s+by] != NULL){
        argv[s] = argv[s+by];
        s++;
    }
    argv[s] = NULL;
     ...
     if ((pid = fork()) < 0) {
         ...
         while(argv[in] != NULL){
             if( strcmp(argv[in], ">") == 0 ){
                 ...
                 int fd = open(argv[in+1], O_WRONLY | O_CREAT, mode  );
                 dup2(fd, 1);
                 close(fd);
                 shiftBy(argv, in, 2);
             }
             else if(startsWith(">", argv[in])){
                 char *tem;
                 tem = argv[in];
                 tem = &tem[1];
                 mode_t mode = S_IRWXU | S_IRWXG;
                 int fd = open(tem, O_WRONLY | O_CREAT, mode );
                 dup2(fd, 1);
                 close(fd);
                 shiftBy(argv, in, 1);
             }
...
void pipeCmds(char **argv) {
    ...
    int pos = -1;
    ind = 0;
    while(argv[ind] != NULL && argv[ind][0] != '|') ind++;
    if(argv[ind] != NULL) pos = ind;
    if(pos != -1) argv[pos] = NULL;
...
pid_t executePipe(char **argv, int inputfd){
    int ind;
    int pos = -1;
    while(argv[ind] != NULL && argv[ind][0] != '|') ind++;
    if(argv[ind] != NULL) pos = ind;
...
pid_t executePipeCover(char **argv, bool towait, bool *running){
    pid_t pid;
    if ((pid = fork()) < 0) {
        printf("*** ERROR: forking child process failed\n");
        exit(1);
...
void executeFromLine(char *lineWithSpace, bool towait){
    ...
    do{
        ...
        if(line[i] == ' ' || line[i] == '\t' || line[i] == '\0' || line[i] == '\n' || line[i] == '|'){
            ...
            if(line[i] == '|'){
                pipeCom = true;
                args[j] = (char*)(malloc(sizeof(char)*(1+1)));
                args[j][0] = '|';
                args[j][1] = '\0';
                j++;
            }
    ...
    bool running = true;
    pid_t process_pid;
    if(pipeCom) process_pid = executePipeCover(args, towait, &running);
    else process_pid = execute(args, towait, &running);
